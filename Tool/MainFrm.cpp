...
#pragma region 복습
	...
	// CreateStatic(부모 윈도우 주소, 행의 수, 열의 수, 창 스타일, 자식창 ID) :분할 창을 만들어주는 함수
	//m_MainSplitter.CreateStatic(this, 2, 2);

	// CreateView : 분할 창에 표시할 뷰 정보를 만드는 함수
	// CreateView(배치할 행, 배치할 열, 배치할 view 창 객체 생성, 초기 크기, pContext)
...
#pragma endregion 복습
	...
	m_SecondSplitter.CreateStatic(&m_MainSplitter, 
									2,
									1,
									WS_CHILD | WS_VISIBLE,		// 창 생성 옵션 WS_CHILD: 자식 창이라는 옵션   WS_VISIBLE : 생성 후 바로 화면에 표시 옵션
									m_MainSplitter.IdFromRowCol(0, 0)); // IdFromRowCol : 지정한 행과 열에 해당하는 창 id값 반환
	

	m_SecondSplitter.CreateView(0, 0, RUNTIME_CLASS(CMiniView), CSize(400, 300),pContext);
	m_SecondSplitter.CreateView(1, 0, RUNTIME_CLASS(CMyForm), CSize(400, 400),pContext);
