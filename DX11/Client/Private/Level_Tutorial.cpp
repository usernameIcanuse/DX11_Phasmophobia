...
HRESULT CLevel_Tutorial::Ready_Layer_SkyBox(const _tchar* pLayerTag)
{
...
HRESULT CLevel_Tutorial::Load_Stage()
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CLevel_Tutorial::Ready_Layer_Terrain(const _tchar* pLayertag)
{
...
HRESULT CLevel_Tutorial::Load_TruckProps()
{
	...
	*/
	...
	while (true)
	{
		if (TRUE == ReadFile(hFile, &tData, sizeof(MAP_DATA), &dwByteHouse, nullptr))
		{
			...
			if (pGameInstance->Add_GameObject(LEVEL_GAMEPLAY, TEXT("Layer_Truck"), TEXT("Prototype_GameObject_KeyBoard"), nullptr, &WorldMatrix))
				return E_FAIL;
		}
	}
	...
	while (true)
	{
		if (TRUE == ReadFile(hFile, &tData, sizeof(MAP_DATA), &dwByteHouse, nullptr))
		{
			...
			if (pGameInstance->Add_GameObject(LEVEL_GAMEPLAY, TEXT("Layer_Truck"), TEXT("Prototype_GameObject_Mouse"), nullptr, &WorldMatrix))
				return E_FAIL;
		}
	}
