...
CZFrustum::CZFrustum()
{
	ZeroMemory( m_planes, sizeof( m_planes[0] ) * 6 );
}


void CZFrustum::Update_Frustum()
{
	_matrix matView = GAMEINSTANCE->Get_Transform(CPipeLine::D3DTS_VIEW);
	_matrix matProj = GAMEINSTANCE->Get_Transform(CPipeLine::D3DTS_PROJ); 
		
	

	Make(300.f, matProj, matView);
	
}
void CZFrustum::Make(float screenDepth, XMMATRIX projectionMatrix, XMMATRIX viewMatrix)
{
...
BOOL CZFrustum::CheckPoint(float x, float y, float z)
{
	for (int i = 0; i < 6; i++)
	{
...
BOOL CZFrustum::CheckCube(float xCenter, float yCenter, float zCenter, float radius)
{
...
BOOL CZFrustum::CheckSphere(float xCenter, float yCenter, float zCenter, float radius)
{
	for (int i = 0; i < 6; i++)
	{
...
BOOL CZFrustum::CheckRectangle(float xCenter, float yCenter, float zCenter, float xSize, float ySize, float zSize)
{
